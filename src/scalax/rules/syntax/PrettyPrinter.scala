...
class PrettyPrinter extends SimpleScalaParser {
  ...
  val escapeItem : SyntaxRule[String] = (scanner.newline -^ "<br />\n"
    | scanner.elem(' ') -^ "&#160;"
    | scanner.elem('&') -^ "&amp;"
    | scanner.elem('<') -^ "&lt;"
  ...
  def span(styleClass : String)(rule : SyntaxRule[String]) = rule ^^ ("<span class=\"" + styleClass + "\">" + _ + "</span>")
  ...
  def prettyPrintFor(rule : SyntaxRule[Any])(input : S) = ((rule&) -~ prettyPrint)(input) match {
